...
void FindScoreMod()
{
	bScoremod = LibraryExists("l4d2_scoremod") || LibraryExists("l4d2_scoremod_cmel");
	bHybridScoremod = LibraryExists("l4d2_hybrid_scoremod") || LibraryExists("l4d2_hybrid_scoremod_zone") || LibraryExists("l4d2_hybrid_scoremod_cul");
	...
	switch (g_Gamemode)
	{
		...
		case GAMEMODE_VERSUS:
		{
			if (bHybridScoremod)
			{
				...
				int medBonus	= SMPlus_GetMedBonus(),   	maxMedBonus 	= SMPlus_GetMaxMedBonus();

				int totalBonus		= healthBonus		+ damageBonus		+ pillsBonus       +medBonus;
				int maxTotalBonus	= maxHealthBonus	+ maxDamageBonus	+ maxPillsBonus    +maxMedBonus;
				...
				FormatEx(	info,
							sizeof(info),
							"> HB: %i | Medkits: %i",
							healthBonus, medBonus);
				DrawPanelText(hSpecHud, info);
				
				FormatEx(info, sizeof(info), "> Bonus: %i", totalBonus);
				DrawPanelText(hSpecHud, info);

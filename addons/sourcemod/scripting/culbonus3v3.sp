...
public OnPluginStart()
{
	...
	hMaxDamageCvar = CreateConVar("sm_max_damage", "405.0");
...
stock DisplayBonus(client=-1)
{
	...
	for (new round = 0; round <= GameRules_GetProp("m_bInSecondHalfOfRound"); round++)
	{
		...
		if (client == -1)
		{
			PrintToChatAll("Map Distance: \x05%d\x01", RoundToFloor(L4D_GetVersusMaxCompletionScore() * 0.75));
			PrintToChatAll("\x01%s: %s", msgPartHdr, msgPartDmg);
		}
		else if (client)
		{
			PrintToChat(client, "Map Distance: \x05%d\x01", RoundToFloor(L4D_GetVersusMaxCompletionScore() * 0.75));
			PrintToChat(client, "\x01%s: %s", msgPartHdr, msgPartDmg);
		}
		else
		{
			PrintToServer("Map Distance: \x05%d\x01", RoundToFloor(L4D_GetVersusMaxCompletionScore() * 0.75));
...
{
   if(GetDamage() < 405)
	  return RoundToFloor((GetConVarFloat(hMaxDamageCvar) - GetDamage()) * iMapDistance / 900 + (GetSurvivorPillBonus() + GetSurvivorMedBonus()) / GetAliveSurvivors());
   else
	...
	for (new i = 1; i <= MaxClients && survivorCount < iTeamSize; i++)
	{
		if (IsSurvivor(i))
		{
			...
			if (IsPlayerAlive(i) && !IsPlayerIncap(i) && HasPills(i))
				pillsBonus += iMapDistance * 0.05 * 0.75;
		}
	}
	return pillsBonus;
}
	...
	for (new i = 1; i <= MaxClients && survivorCount < iTeamSize; i++)
	{
		if (IsSurvivor(i))
		{
			...
			if (IsPlayerAlive(i) && !IsPlayerIncap(i) && HasMeds(i))
				medsBonus += iMapDistance * 0.1 * 0.75;
		}
	}
	return medsBonus;
}
...
void OnPlayerIncapped(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsSurvivor(victim))
	    iTotalDamage[GameRules_GetProp("m_bInSecondHalfOfRound")] += 45;
}